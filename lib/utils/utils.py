...
def load_model_state(model, output_dir, epoch):
    file = os.path.join(output_dir,
                        'checkpoint_3d_epoch'+str(epoch)+'.pth.tar')
...
def load_checkpoint(model, optimizer,
    ...
    if os.path.isfile(file):
        ...
        return start_epoch, model, checkpoint, optimizer, precision

    else:
        print('=> no checkpoint found at {}'.format(file))
        return 0, model, None, optimizer, 0


def load_checkpoint_best(model, optimizer,
                         output_dir, filename='checkpoint.pth.tar'):
    file = os.path.join(output_dir, filename)
    if os.path.isfile(file):
        checkpoint = torch.load(file, map_location='cpu')
...
def save_checkpoint(states, is_best, output_dir,
    ...
    os.system("cp {} /content/drive/MyDrive/mvp/logs".format(os.path.join(output_dir, filename)))
    if is_best and 'state_dict' in states:
        torch.save(states['state_dict'],
                   os.path.join(output_dir, 'model_best.pth.tar'))
        os.system("cp {} /content/drive/MyDrive/mvp/logs".format(os.path.join(output_dir, 'model_best.pth.tar')))


def load_backbone_panoptic(model, pretrained_file):
    this_dir = os.path.dirname(__file__)
    pretrained_file = \
        os.path.abspath(os.path.join(this_dir, '../..', pretrained_file))
    ...
    for k, v in pretrained_state_dict.items():
        if k.replace(prefix, "") in model_state_dict \
                and v.shape == model_state_dict[k.replace(prefix, "")].shape:
            ...
            o = torch.zeros_like(
                model_state_dict[k.replace(prefix, "")][:, :, :, :])
